...
function DualCertificate() {
  ...
  const {
    ...
    handleMouseDown,
    handleResizeStart
  } = useCertificateManager("STCW", "BST_VERIFICATION.pdf");
  ...
  const handleFetchName = async () => {
    ...
    const certificateName = "Basic Safety Training (STCW)";
    ...
    try {
      ...
      const response = await fetch('http://127.0.0.1:5000/save-certificate-data', {
        method: 'POST',
        headers: {
  ...
  // Check if STCW course was selected and fetch candidate data
  useEffect(() => {
    const stcwVisited = localStorage.getItem("status_STCW");
    const selectedCourse = localStorage.getItem("selectedCourse");

    if (stcwVisited === "true") {
      setSelectedCourse("STCW");
      console.log(`[COURSE] STCW certificate page loaded. Selected course: ${selectedCourse}`);
      fetchCandidateData();
    }
  }, []);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (response.status === 404) {
          console.warn("[CANDIDATE] No current candidate data found:", errorResult.message || "File not found");
          setCandidateData(null);
        } else if (response.status === 400) {
          console.warn("[CANDIDATE] Incomplete candidate data:", errorResult.message || "Missing required fields");
          setCandidateData(null);
        } else {
          console.error("[CANDIDATE] Failed to fetch current candidate data:", response.status, errorResult.message || "Unknown error");
          setCandidateData(null);
        }
      }
    } catch (error) {
  ...
  const drawCandidateTextLeft = (ctx, data) => {
    ...
    console.log("[CANVAS] Drawing candidate text on BST verification LEFT canvas with text wrapping");
    ...
    // BST Certificate - Separate coordinates for name and passport
    // Name field coordinates: (310, 158)
    const nameLines = wrapText(ctx, fullName, 310, 158, 19, 20);

    // Passport field coordinates: (310, 210) - separate position, no overlap
    wrapText(ctx, data.passport, 310, 210, 19, 20);

    console.log("[CANVAS] LEFT BST - Name at (310, 158), Passport at (310, 210)");
  ...
  const drawCandidateTextRight = (ctx, data) => {
    ...
    console.log("[CANVAS] Drawing candidate text on BST verification RIGHT canvas WITHOUT text wrapping");
    ...
    console.log("[CANVAS] RIGHT BST - Text drawn without wrapping - Name:", fullName, "Nationality:", data.nationality, "DOB:", data.dob);
  };
  ...
  useEffect(() => {
    ...
    leftImage.onload = () => {
      ...
      // Only draw candidate text if STCW course was selected
    ...
    rightImage.onload = () => {
      ...
      // Draw candidate text on right canvas if STCW course was selected
      if (selectedCourse && candidateData) {
