...
function DualCertificate4() {
  ...
  const {
    ...
    handleMouseDown,
    handleResizeStart
  } = useCertificateManager("BOSIET", "BOSET_VERIFICATION.pdf");
  ...
  // Check if BOSIET course was selected and fetch candidate data
  useEffect(() => {
    const bosietVisited = localStorage.getItem("status_BOSIET");
    const selectedCourse = localStorage.getItem("selectedCourse");

    if (bosietVisited === "true") {
      setSelectedCourse("BOSIET");
      console.log(`[COURSE] BOSIET certificate page loaded. Selected course: ${selectedCourse}`);
      fetchCandidateData();
    }
  }, []);
  ...
  const fetchCandidateData = async () => {
    try {
      console.log("[CANDIDATE] Fetching current candidate data for BOSIET course...");
  ...
  const drawCandidateTextLeft = (ctx, data) => {
    ...
    console.log("[CANVAS] Drawing candidate text on BOSIET verification LEFT canvas with text wrapping");
    ...
    // BOSIET Certificate - Separate coordinates for name and passport
    // Name field coordinates: (310, 115)
    const nameLines = wrapText(ctx, fullName, 310, 115, 19, 20);

    // Passport field coordinates: (310, 163) - separate position, no overlap
    wrapText(ctx, data.passport, 310, 163, 19, 20);

    console.log("[CANVAS] LEFT BOSIET - Name at (310, 115), Passport at (310, 163)");
  ...
  const drawCandidateTextRight = (ctx, data) => {
    ...
    console.log("[CANVAS] Drawing candidate text on BOSIET verification RIGHT canvas WITHOUT text wrapping");
    ...
    console.log("[CANVAS] RIGHT BOSIET - Text drawn without wrapping - Name:", fullName, "Nationality:", data.nationality, "DOB:", data.dob);
  };
  ...
  useEffect(() => {
    ...
    leftImage.src = '/static/BOSET VERIFICATION.jpg';
    rightImage.src = '/static/angelmaritime4.jpg';
    ...
    leftImage.onload = () => {
      ...
      // Only draw candidate text if BOSIET course was selected
    ...
    rightImage.onload = () => {
      ...
      // Draw candidate text on right canvas if BOSIET course was selected
      if (selectedCourse && candidateData) {
        ...
        >
          ...
          <div className="px-4 py-2 rounded-lg text-sm font-medium bg-blue-100 text-blue-800 border border-blue-200">
            <span className="flex items-center gap-2">
              📚 Course: BOSIET Safety Training Certificate
            </span>
          </div>
...
export default DualCertificate4;
