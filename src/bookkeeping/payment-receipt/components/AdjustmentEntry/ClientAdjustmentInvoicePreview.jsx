...
function ClientAdjustmentInvoicePreview() {
  ...
  useEffect(() => {
    if (adjustmentData && companyData && customerData && !loading) {
      autoSaveInvoice();
    }
  }, [adjustmentData, companyData, customerData, loading]);
          ...
          try {
            ...
            const response = await fetch('http://localhost:5000/api/files/save-invoice-image', {
              ...
              body: JSON.stringify({
                ...
                image_type: 'pdf',
                file_name: `Client_Adjustment_Invoice_${invoiceNo}.pdf`,
                voucher_type: 'Adjustment'
              }),
            });

            if (response.ok) {
              console.log('Client adjustment invoice automatically saved to database successfully');
            } else {
              console.error('Failed to auto-save client adjustment invoice to database');
            }
          } catch (saveError) {
            console.error('Error auto-saving client adjustment invoice to database:', saveError);
          }
        };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.status === 'success' && result.data?.image_data) {
          ...
          link.href = url;
          link.download = result.data.file_name || `Client_Adjustment_Invoice_${invoiceNo}.pdf`;
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
          URL.revokeObjectURL(url);

          console.log('Downloaded existing client adjustment invoice from database');
          return;
        }
      }

      // Fallback: Generate new PDF if not found in database
      console.log('Client adjustment invoice not found in database, generating new PDF');
      const element = document.querySelector('.adjustment-invoice-content');
      if (element) {
        const clonedElement = element.cloneNode(true);
        clonedElement.style.boxShadow = 'none';
