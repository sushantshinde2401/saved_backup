...
function DualCertificate2() {
  ...
  const {
    ...
    handleMouseDown,
    handleResizeStart
  } = useCertificateManager("STSDSD", "STSDSD_VERIFICATION.pdf");
  ...
  // Check if STSDSD course was selected and fetch candidate data
  useEffect(() => {
    const stsdsdVisited = localStorage.getItem("status_STSDSD");
    const selectedCourse = localStorage.getItem("selectedCourse");

    if (stsdsdVisited === "true") {
      setSelectedCourse("STSDSD");
      console.log(`[COURSE] STSDSD certificate page loaded. Selected course: ${selectedCourse}`);
      fetchCandidateData();
    }
  }, []);
  ...
  const fetchCandidateData = async () => {
    try {
      console.log("[CANDIDATE] Fetching current candidate data for STSDSD course...");
  ...
  const drawCandidateTextLeft = (ctx, data) => {
    ...
    console.log("[CANVAS] Drawing candidate text on STSDSD verification LEFT canvas with text wrapping");
    ...
    // STSDSD Certificate - Separate coordinates for name and passport
    // Name field coordinates: (310, 156)
    const nameLines = wrapText(ctx, fullName, 310, 156, 19, 20);

    // Passport field coordinates: (310, 210) - separate position, no overlap
    wrapText(ctx, data.passport, 310, 210, 19, 20);

    console.log("[CANVAS] LEFT STSDSD - Name at (310, 156), Passport at (310, 210)");
  ...
  const drawCandidateTextRight = (ctx, data) => {
    ...
    console.log("[CANVAS] Drawing candidate text on STSDSD verification RIGHT canvas WITHOUT text wrapping");
    ...
    console.log("[CANVAS] RIGHT STSDSD - Text drawn without wrapping - Name:", fullName, "Nationality:", data.nationality, "DOB:", data.dob);
  };
  ...
  useEffect(() => {
    ...
    leftImage.src = '/static/STSDSD VERIFICATION.jpg';
    rightImage.src = '/static/angelmaritime2.jpg';
    ...
    leftImage.onload = () => {
      ...
      // Only draw candidate text if STSDSD course was selected
    ...
    rightImage.onload = () => {
      ...
      // Draw candidate text on right canvas if STSDSD course was selected
      if (selectedCourse && candidateData) {
        ...
        >
          ...
          <div className="px-4 py-2 rounded-lg text-sm font-medium bg-blue-100 text-blue-800 border border-blue-200">
            <span className="flex items-center gap-2">
              📚 Course: STSDSD Verification Certificate
            </span>
          </div>
...
export default DualCertificate2;
